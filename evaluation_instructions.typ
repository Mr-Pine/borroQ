...
#let test-case-mapping = (
  ...
  immut-return: (
    "ImmutableReturn",
    [The return value of a method whose return type is annotated with #mutable, we can neither assign it to a local variable declared as #mutable (`foo`), nor can we reassign a field if we assign it to an #immutable variable.],
  ),
  alia-detection: (
    "AliasDetection",
    [Since `a` is aliased by `b` (and `b` is kept live by the print), we are not allowed to reassign a field on `a`.],
  ),
  field-alias-detection: (
    "FieldAliasDetection",
    [Since `a.b` is aliased by `b` (and `b` is kept live by the print), we are not allowed to reassign the field `xy` on `a.b`.],
  ),
  deep-field: ("DeepField", [Since the field `b` on the class `A` is declared as #immutable, we cannot reassign any fields on it.]),
  scoping: ("Scope", [Since `getB` is scoped to the field `b`, we can still reassign `xy` on the other field `a`.]),
  ...
  recomb: ("Recombination", [After the alias `b` is not live anymore, we can recombine the permissions, allowing us to reassign `xy` on `a`.]),
